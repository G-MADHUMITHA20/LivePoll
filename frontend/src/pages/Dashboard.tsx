    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto min-h-[calc(100vh-73px)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Manage your active and past polls.</p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map(n => (
            <div key={n} className="card h-48 animate-pulse bg-white dark:bg-gray-800">
        ...
        <div className="card flex flex-col items-center justify-center py-20 text-center border-dashed border-2 border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
          <div className="w-16 h-16 bg-white dark:bg-gray-700 rounded-full flex items-center justify-center shadow-sm mb-4 text-gray-400 dark:text-gray-300">
          ...
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">No polls created yet</h3>
          <p className="text-gray-500 dark:text-gray-400 max-w-sm mx-auto mb-6">Create your first poll to start gathering real-time feedback from your audience.</p>
          ...
          {polls.map(poll => {
            ...
            <div key={poll.id} className="card hover:shadow-md transition-all flex flex-col border-gray-200 group">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-xs font-medium text-gray-400 dark:text-gray-500 bg-gray-50 dark:bg-gray-800 px-2 py-1 rounded-md">
                  {new Date(poll.created_at).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                </span>
              </div>
              <h3 className="font-bold text-lg mb-2 text-gray-900 dark:text-white line-clamp-2 leading-snug group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                {poll.question}
              </h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 flex items-center gap-1.5 font-medium">
              ...
              <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                <Link to={`/manage/${poll.id}`} className="text-primary-600 dark:text-primary-400 text-sm font-semibold hover:text-primary-700 dark:hover:text-primary-300 hover:underline flex items-center gap-1">
                  Manage Details
                </Link>
                <Link to={`/poll/${poll.id}`} className="text-gray-500 dark:text-gray-400 text-sm font-semibold hover:text-gray-900 dark:hover:text-white bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700 px-3 py-1.5 rounded-lg transition-colors border border-gray-200 dark:border-gray-600">
                  Open Live
                </Link>
              </div>
            </div>
